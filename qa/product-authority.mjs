...
const expected = [
  ...
  { collection: 'Core', identifier: '07', name: 'CALABRIAN GLOW' },
...
if (
  !Array.isArray(intake.products) ||
  intake.products.length !== expected.length
) {
...
expected.forEach((item, index) => {
  ...
  for (const field of ['collection', 'identifier', 'name']) {
    if (actual[field] !== item[field]) {
      fail(
        `position ${index + 1} ${field} expected "${item[field]}" but found "${actual[field]}"`,
      );
    }
  }
  ...
  if (!master.includes(item.identifier) || !master.includes(item.name)) {
    fail(
      `master authority does not contain locked identity ${item.identifier} / ${item.name}`,
    );
  }
});
...
for (const forbidden of forbiddenActiveIdentities) {
  if (
    intake.products.some(
      (product) =>
        product.name === forbidden || product.identifier === forbidden,
    )
  ) {
    fail(
      `historical descriptive label reintroduced as active identity: ${forbidden}`,
    );
  }
}
...
for (const [identifier, sourceScale] of limitedSourceScales) {
  if (!master.includes(`${identifier} — ${sourceScale} — Extreme`)) {
    fail(
      `${identifier} source heat authority ${sourceScale} is missing from ${masterPath}`,
    );
  }
}
...
console.log(
  `Validated ${expected.length} locked identities and runtime heat mappings against ${masterPath} and catalogue intake.`,
);
